...
- Real-agent evaluation consumes the exact `rawBaseline.contextText`/`warmRetrieval.contextText` the experiment already measured; providers never independently run my-dev-kit retrieval. The real-agent prompt explicitly forbids repository inspection, filesystem search, my-dev-kit execution, shell commands, external information, and file modification, and delivers the prompt over stdin. Each provider call runs from a neutral temporary working directory, never the repository, benchmark target, installed package, or agent artifact directory.
- Added Codex JSONL and Claude JSON response parsing to the existing agent adapters, sharing the existing prompt/parser/classifier/scorer pipeline with the deterministic fake agent.
- Extended provider outcome classification with `agent-unavailable`, `agent-limit-reached`, and `timeout`, alongside the existing `completed`, `failed`, `invalid-output`, and `skipped`. Warm-index infrastructure status (index build, raw/warm retrieval) and agent/provider outcome status are reported and reasoned about separately: infrastructure can report `completed` while campaign evidence is `partial` or `unavailable`.
- Extended the warm-index report (schema unchanged: `my-dev-kit-lab-warm-index-report-v1`) with agent identity (`agent.id`, `agent.mode`) and an additive campaign summary (preset, provider, timeout, selected/scheduled/executed/not-run-for-missing-context side counts, closed provider outcome counts, `agentEvidenceStatus`, `tokenEvidenceStatus`). Generic agent metric IDs (`agent-correctness-score`, `agent-total-tokens`, `cumulative-agent-total-tokens`) and their formulas are unchanged; correctness remains deterministic answer-key scoring and missing token evidence remains explicitly unavailable, never zero, never substituted with an estimated context-token count. The existing four warm-index plots are unchanged in formula and gained provider-aware titles/labels; a v1 report from before this change still plots correctly as legacy fake-agent evidence.
- A real-agent campaign run now automatically produces, beneath its output root: the existing report (`report.json`/`.txt`/`.html`), the same four warm-index SVG charts under `plots/`, a best-effort `report.png` (skipped without a Playwright/browser runtime — non-fatal — and reported as an unsuccessful command only on an unexpected capture failure, which still preserves the report/plots/gallery already written), and a narrow three-item campaign gallery (`gallery/gallery-manifest.json`, `gallery-index.html`: `warm-index-campaign-report`, `warm-index-campaign-plots`, `warm-index-execution`) with relative, bounded paths. Legacy non-campaign warm-index runs remain report-only. The `warm-index-reuse` plugin itself owns no presentation orchestration; the command layer sequences report → plots → screenshot → gallery.
- Hardened `runMeasuredCommand`'s command-timeout path: a bounded fallback now resolves the measured-command promise even if a killed process's descendant outlives an intermediate shim, and its streams are explicitly detached so the calling process can exit; a provider evaluation's neutral-working-directory cleanup is now best-effort so an OS-level cleanup failure never masks already-classified agent evidence.
- Extended the packed-package installed-execution acceptance gate (`npm run verify:packed-package`) with deterministic, cross-platform, local-only Codex/Claude provider fixtures proving, through the installed package: completed-with-tokens, token-unavailable, failed, invalid-output, agent-unavailable, agent-limit-reached, and timeout provider outcomes; the `codex-timeout-isolation` preset's continuation past a failed case; captured and skipped screenshot paths; the three-item campaign gallery; provider-argv privacy (no prompt/context in argv, frozen stdin transport flags); and target/installed-package immutability. No real Codex or Claude is ever invoked by this gate.
...
- v0.4.7 was released after cross-platform CI, security validation, code-rot audit, package validation, and packed-package acceptance passed.
