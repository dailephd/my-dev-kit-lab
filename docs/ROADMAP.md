...
1. **Action contract and runtime — completed.**
   * Added the value-only `select-option` action to `src/tutorial/types.ts` and to the closed action-type list.
   * Added closed validation in `src/tutorial/scenarioValidation.ts` covering locator, required non-empty value, optional timeout, and unknown-field rejection.
   * Added only the narrow `selectOption` call shape to the minimal structural Playwright locator surface in `src/browser/types.ts`.
   * Executed the action in `src/tutorial/tutorialActions.ts` through the canonical locator resolver, with exact returned-selected-value verification.
   * Routed the synthetic cursor to the select element before the semantic action in `src/tutorial/tutorialSession.ts`, with no click-ripple and no native-menu animation.
   * Added unit, session, and security regression coverage for valid selection, missing/empty/whitespace value, unknown fields, invalid timeout, rejected label/index/multi-select shapes, timeout propagation, Playwright failure propagation, returned-value verification, absence of retry and fallback, cursor movement, and unchanged step-failure semantics.

2. **Generic real-browser and exact-package portability proof — completed.**
   * Extended `examples/tutorial-browser/` with one real native `<select>` (accessible name `Operation`), stable option values, and deterministic visible state updated by an ordinary `change` event.
   * Extended the canonical scenario to ten steps with one `select-option` step that chooses `preserve` and verifies the resulting visible state through existing assertion types.
   * Extended `tests/integration/tutorialRealBrowser.spec.ts` so real Chromium proves the selected value, the live DOM state, and the generated artifacts.
   * Extended `scripts/verify-packed-package.mjs` with the `TUTORIAL_SELECT_OPTION` gate, so the exact npm tarball installed into a clean consumer validates and executes `select-option`, records it in the tutorial manifest, preserves source/target/package immutability, and still proves every pre-existing tutorial action.
   * Reconciled `tests/scripts/packageScriptContract.spec.ts`, the second owner of the canonical scenario's action inventory, to the ten-step scenario.
   * Completed a bounded read-only downstream Observer compatibility check and confirmed the generic value-only contract expresses its representative native selects.
   * Passed the existing cross-platform CI matrix on Ubuntu, macOS, and Windows across Node 24 and Node latest.
