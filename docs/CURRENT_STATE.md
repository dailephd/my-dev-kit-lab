...
- Active planned version: `v0.4.9` (semantic native-select option selection for browser tutorials); `v0.5.0` warm-index reuse remains the next planned version after this bounded patch
...
- Workflow stage: `v0.4.8` is the current release; `v0.4.9` is the active planned patch and has not started implementation; `v0.5.0` follows after it.
...
- Release blockers for the published `v0.4.8`: none. A downstream portability defect discovered during `my-frontend-observer` v0.9 release readiness is now assigned to planned `v0.4.9`: native HTML `<select>` controls need a semantic value-selection action instead of keyboard navigation. Observer's separate tutorial-pacing mismatch and `contractClass` readiness-verifier crash remain Observer-owned.
- Exact next action: implement `v0.4.9` Batch 1 (value-only `select-option` contract, validation, runtime, cursor behavior, and focused tests), then Batch 2 (generic native-select fixture, real Chromium coverage, exact packed-package acceptance, and bounded read-only Observer compatibility evidence).
...
- `v0.4.9` is planned, not implemented: add one value-only `select-option` action for native HTML `<select>` controls through the existing `TutorialLocatorV1` resolver and Playwright `Locator.selectOption({ value })`; keep tutorial schema/result/manifest version `1.0.0`, preserve `press` as real keyboard input, add no keyboard fallback, and prove portability through the existing real-browser and packed-package gates on Windows, Linux, and macOS.
...
- `v0.4.9` value-only semantic native-select tutorial action (`select-option`), generic real-browser fixture coverage, cross-platform portability proof, and exact packed-package acceptance
...
- The released v0.4.8 tutorial vocabulary has no semantic action for choosing a native HTML `<select>` option. Downstream keyboard sequences such as `ArrowDown` plus `Enter` are therefore not considered a portable substitute; planned v0.4.9 owns the bounded value-only `select-option` correction.
