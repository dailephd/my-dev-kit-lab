...
The current framework keeps automated security validation, the generic audit framework, language-aware code-rot analysis, and Android validation as distinct responsibilities. Their release history is recorded in [CHANGELOG.md](../CHANGELOG.md); v0.4.6 is the latest published release. v0.4.6 does not change any security check, verdict rule, or evidence semantic described in this document — it adds an installed CLI route (`my-dev-kit-lab security validate`) that calls the same standalone command owner as `npm run security:validate`, with implicit report output rooted under the installed CLI's writable workspace instead of the package root. `npm run security:validate` from a source checkout remains available alongside the installed CLI route.
