...
`v0.3.3` is the previously published npm baseline. It keeps the `v0.2.2` security-validation fortification and the `v0.3.0` generic audit framework as a separate tool, and builds on the `v0.3.1` language-aware TypeScript/JavaScript code-rot substrate and the `v0.3.2` security-validation audit adapter. `v0.3.3` extends code-rot support to Java/Kotlin, but it does not add Java/Kotlin security validation, Android validation, or any new `security:validate` behavior. `v0.3.4` is release-prepared (package metadata bumped to `0.3.4`, not yet published) and does not change security command names or the security report schema.
