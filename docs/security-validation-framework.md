...
`v0.3.2` is the current published npm baseline. It keeps the `v0.2.2` security-validation fortification and the `v0.3.0` generic audit framework as a separate tool, adds the `v0.3.1` language-aware TypeScript/JavaScript code-rot substrate, and adds a security-validation audit adapter. The checked-out, release-prepared `v0.3.3` work extends code-rot support to Java/Kotlin, but it does not add Java/Kotlin security validation, Android validation, or any new `security:validate` behavior.
