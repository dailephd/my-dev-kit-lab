...
The current published baseline is `v0.3.4`. The work on `release/v0.4.0` is **release-prepared; publication pending**. It is not published, and the npm registry latest version remains `0.3.4`.
...
1. `v0.4.0`: Android validation MVP — release-prepared; publication pending.
