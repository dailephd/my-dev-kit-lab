...
After v0.9.2, the planned v0.10.x-v0.12.x track expands repository review across six dimensions: **behavior**, **architecture**, **security**, **operations**, **quality**, and **evolution**. v0.10.x establishes shared architecture evidence, deterministic topology analysis, and extensibility/reuse findings. v0.11.x implements the quality audit type plus behavior/test and evolution/change-cost evidence. v0.12.x adds non-security operational-quality evidence, unified project review, and benchmark/calibration evidence before v1.0.0.

These dimensions are a review model over shared evidence, not six independent engines. The generic audit runner, source facts, future architecture evidence, issue/report contracts, and command owners should be reused. Security remains independently authoritative: project review may consume confirmed findings through the existing security audit adapter, but must not duplicate security validation, Android checks, attack scenarios, or security verdict policy.
