...
Code-quality audit, project-wide combined audit defaults, framework-aware profiles, JVM package/environment rot, Gradle/Maven dependency freshness checks, and manual pentest remain future roadmap work. The planned v0.10.x-v0.12.x software-review track adds architecture evidence/topology, extensibility and reuse analysis, quality, behavior/test evidence, evolution/change-cost evidence, operational quality, unified project review, and calibration; none of those future capabilities are current v0.4.8 behavior.
...
Version v0.4.3 evaluates stage-specific bounded repository context and workflow instructions through the existing experiment infrastructure and is published. Version v0.4.4 added the producer-readiness bridge through programmatically configured inputs while preserving upstream producer and readiness ownership. Version v0.4.5 evaluates agreement between condition-aware producer evidence and orchestrator run-integrity evidence against a frozen, deterministic, hash-verified regression fixture pair. Version v0.4.6 added the installed-package CLI and runtime-boundary correction. Version v0.4.7 introduced the generic persistent-browser runtime, managed demo processes, bounded scenario actions and assertions, visible tutorial overlays, WebM recording, synchronized screenshots/subtitles/Markdown, a tutorial manifest, and clean-consumer package acceptance while keeping product-specific demo sites and scenarios in their owning repositories. Version v0.4.8 completes a missing visual-editor interaction class by adding locator-anchored fraction `pointer-click` and `pointer-drag` using real Playwright mouse input, keeping existing `drag` unchanged, extending the generic real-browser/packed-package fixture, and preserving all current safety boundaries. Version v0.5.0 follows next: before `warm-index-reuse` becomes the second registered experiment plugin, it generalizes the shared experiment-run command with a versioned `--config` envelope and plugin-owned input resolution so future plugins do not add command-owner special cases.

The existing v0.5.x-v0.9.x sequence remains intact: warm-index reuse, freshness and affected-neighborhood evidence, scale/synthetic/external repositories, retrieval precision and context packs, agent success, normalized telemetry, prompt hardening, tutorial-manifest gallery consumption, and generalized reporting. Those releases provide important prerequisites for later software-review work rather than being replaced by it.

After v0.9.2, the planned v0.10.x-v0.12.x track expands repository review across six dimensions: **behavior**, **architecture**, **security**, **operations**, **quality**, and **evolution**. v0.10.x establishes shared architecture evidence, deterministic topology analysis, and scenario-based extensibility/reuse findings. v0.11.x implements the quality audit type plus behavior/test and evolution/change-cost evidence. v0.12.x adds non-security operational-quality evidence, unified project review, and benchmark/calibration evidence before v1.0.0.

The measurement model is informed by established software-quality work rather than an invented universal health score: ISO/IEC 25010 supplies a product-quality reference model; ISO/IEC 25023 supplies product-quality measurement guidance and explicitly leaves acceptable ranges context-dependent; ISO/IEC 5055/CISQ supplies automated structural source-code measures for Security, Reliability, Performance Efficiency, and Maintainability; other planned measures use published or established definitions such as cyclomatic complexity, CK object-oriented metrics, coverage, mutation score, churn, and co-committal metrics. Exact sources and URLs are recorded in [METRICS.md](METRICS.md).

The six lab dimensions describe the complete `all` software-review view over shared evidence, not six independent engines and not a claim that they reproduce the complete ISO/IEC 25010 taxonomy. The `project` audit type contributes architecture, behavior, evolution, and operations; quality and security keep their separate owners and join the six-dimension view only through the aggregate. The generic audit runner, source facts, future architecture evidence, issue/report contracts, and command owners should be reused. Security remains independently authoritative: project review may consume confirmed findings through the existing security audit adapter, but must not duplicate security validation, Android checks, attack scenarios, CVSS evidence, or security verdict policy.

All of these later items remain planned. Manual pentest remains a human-led post-v1/version-TBD workflow and is not part of the current automated validation system.
