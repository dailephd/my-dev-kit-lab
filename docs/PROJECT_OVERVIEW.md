...
The six lab dimensions are a product/reporting model over shared evidence, not six independent engines and not a claim that they reproduce the complete ISO/IEC 25010 taxonomy. The generic audit runner, source facts, future architecture evidence, issue/report contracts, and command owners should be reused. Security remains independently authoritative: project review may consume confirmed findings through the existing security audit adapter, but must not duplicate security validation, Android checks, attack scenarios, CVSS evidence, or security verdict policy.
