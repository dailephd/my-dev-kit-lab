...
The latest published release is v0.4.5 (context-integrity validation). See [CURRENT_STATE.md](CURRENT_STATE.md) for operational details, [CHANGELOG.md](../CHANGELOG.md) for release history, and [ROADMAP.md](ROADMAP.md) for future scope.
...
Version v0.4.3 evaluates stage-specific bounded repository context and workflow instructions through the existing experiment infrastructure and is published. Version v0.4.4 added the producer-readiness bridge through programmatically configured inputs while preserving upstream producer and readiness ownership. Version v0.4.5, the current published release, evaluates agreement between condition-aware producer evidence and orchestrator run-integrity evidence against a frozen, deterministic, hash-verified regression fixture pair. Version v0.4.6, an installed-package CLI and runtime-boundary correction, is implemented on the development branch but not yet published — it adds a supported installed `my-dev-kit-lab` CLI, separates the installed package root, writable lab workspace, and inspected target root, and adds a packed-tarball installation/execution acceptance gate, without changing any existing security, audit, experiment, or report behavior. The next planned feature version after v0.4.6 publishes is v0.5.0, warm-index reuse. Later planned work covers freshness and stale-index detection, context-window scaling, retrieval precision and recall, agent success, normalized telemetry, scheduling, prompt hardening, and a generalized evidence portal.
