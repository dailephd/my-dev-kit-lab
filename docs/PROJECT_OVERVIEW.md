...
The current tutorial capability executes `TutorialScenarioV1` scenarios through a persistent Playwright browser session against a trusted local target contract. It supports bounded actions and assertions, visual overlays, WebM recording, step screenshots, SRT/VTT subtitles, Markdown, and a versioned tutorial manifest. In v0.4.7, tutorial automation was introduced with element-to-element `drag`. Released v0.4.8 completes a missing visual-editor interaction class by adding bounded `pointer-click` and `pointer-drag` actions anchored to one existing locator with normalized fraction coordinates, while retaining the no-arbitrary-JavaScript/no-page-coordinate boundary. It still has no semantic action for choosing an option in a native HTML `<select>`; keyboard navigation remains real keyboard input and is not a portable substitute for semantic selection. Planned v0.4.9 adds a value-only `select-option` action through the existing locator/action/session owners and Playwright `Locator.selectOption`, without adding arbitrary JavaScript, index-based selection, multi-select, or product-specific behavior. The generic packaged example is owned by this lab; product-specific demo sites and scenario definitions remain in their owning repositories.
