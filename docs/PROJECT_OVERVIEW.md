...
The planned security-validation framework is a parallel release-security track. It does not replace the generic experiment-plugin roadmap and it is not presented as a standard user-facing my-dev-kit capability. Instead, it gives the lab a way to produce repeatable release evidence showing whether the local CLI/package still respects its intended safety boundaries before release preparation.

See [security-validation-framework.md](security-validation-framework.md) for the planned framework details.
