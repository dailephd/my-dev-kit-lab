...
The latest published release is v0.4.6 (installed-package CLI and runtime-boundary correction). The v0.4.7 implementation adds declarative browser tutorials and is documented as implementation-complete, with pre-release readiness still pending. The v0.4.7 implementation is not published. The v0.4.5 context-integrity evaluation remains intentionally frozen against the published `@dailephd/my-dev-kit@1.10.4` and `@dailephd/my-dev-kit-orchestrator@1.2.3` contracts; those versions are historical validation baselines, not a statement that they are the ecosystem's current releases. See [docs/CURRENT_STATE.md](docs/CURRENT_STATE.md).
