...
The latest release is v0.5.2 (warm-index real-agent campaigns), and the package and installed CLI are version 0.5.2. The previous release, v0.5.1, added the expanded warm-index benchmark suite; v0.6.0 (index freshness and changed-file detection) is implemented in the current source branch but unreleased (the published package and installed CLI remain 0.5.2 and do not contain it), and v0.6.1 (affected-neighborhood experiments) is next planned. The v0.4.5 context-integrity evaluation remains intentionally frozen against the published `@dailephd/my-dev-kit@1.10.4` and `@dailephd/my-dev-kit-orchestrator@1.2.3` contracts; those versions are historical validation baselines, not a statement that they are the ecosystem's current releases. See [docs/CURRENT_STATE.md](docs/CURRENT_STATE.md) for current state.
...
- **Record index freshness evidence (v0.6.0, implemented; unreleased):** in the current source branch, a warm-index run captures an index snapshot after its one index build — the exact indexed file set from the my-dev-kit index contract, each with SHA-256 content identity, size, and modified-time metadata, plus the my-dev-kit version and index-command evidence — and assesses freshness immediately before each task's warm retrieval as `fresh`, `stale`, `partially-stale`, or `unknown`. The evidence appears in `warm-index-execution.json` and in the existing `report.json`/`report.txt`/`report.html` warm-index reports. It is observational only: it covers only files the snapshot lists (not the whole repository), never changes retrieval, execution or provider status, and adds no CLI flag, metric, plot, gallery item, or reindex recommendation. See [docs/METRICS.md](docs/METRICS.md#index-freshness-evidence-v060) and [docs/WORKFLOWS.md](docs/WORKFLOWS.md#warm-index-reuse-experiment).
...
| Warm-index execution evidence | `<experiment out>/warm-index-execution.json` (plus `indexes/`, `commands/`, and `agents/` beneath the same output root); the current unreleased v0.6.0 artifact also carries project-level `indexSnapshot` and per-task `indexFreshness` evidence |
