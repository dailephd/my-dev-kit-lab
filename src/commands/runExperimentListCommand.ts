...
import type { ExperimentPlugin, ExperimentPluginMetadata } from "../experiments/index.js";

// ---------------------------------------------------------------------------
// v0.4.6 Batch 4 -- reusable experiment-list command owner.
//
// Extracted from scripts/experiments/listExperiments.ts so both the npm
// script and the installed CLI router call the same registry-listing path.
// Read-only: no filesystem writes, no packageRoot/workspaceRoot dependency.
// ---------------------------------------------------------------------------
...
export async function runExperimentListCommandFromArgs(argv: string[]): Promise<number> {
