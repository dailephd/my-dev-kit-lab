...
export async function runPackageChecks(options: {
  ...
  // npm pack --dry-run writes the file list to stdout on some versions,
  // and to stderr on others. Prefer stdout; fall back to stderr.
  const packOutput = cmd.stdout.trim() ? cmd.stdout : cmd.stderr;
