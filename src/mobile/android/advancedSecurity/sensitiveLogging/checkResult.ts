...
import { analyzeSensitiveLoggingFile } from "./analyzeSensitiveLogging.js";

// ---------------------------------------------------------------------------
// v0.4.1 Batch 6 — standalone Android sensitive-logging check result.
...
export const ANDROID_SENSITIVE_LOGGING_AUDIT_CHECK_ID = "android-sensitive-logging-audit";

export function auditAndroidSensitiveLogging(targetRoot: string, detection: AndroidDetectionResult): AndroidCheckResult {
  if (detection.projectKind === "non-android") {
    return {
      id: ANDROID_SENSITIVE_LOGGING_AUDIT_CHECK_ID,
      category: "android-sensitive-logging",
      title: "Android sensitive-logging audit",
      status: "unsupported",
      requirementLevel: "optional",
      ran: false,
      skipped: true,
      skipInfo: {
        checkId: ANDROID_SENSITIVE_LOGGING_AUDIT_CHECK_ID,
  ...
  for (const file of sourceFiles) {
    const result = analyzeSensitiveLoggingFile(targetRoot, file);
  ...
  return {
    ...
    warnings: [
      ...
      "Static analysis only: does not prove log calls executed, that logcat is accessible, or that Crashlytics uploaded data.",
    ],
    errors: [],
