...
describe("security:validate verdict/fail-on integration against a controlled fixture", () => {
  ...
  it("optional skipped tools alone still report ready-except-optional when mandatory checks pass", async () => {
    const summary = await runSecurityValidation({
      cwd: toolRoot,
  ...
  it("isFullReleaseGate is false for a scoped run (deps,package only)", async () => {
    const scoped = await runSecurityValidation({
      cwd: toolRoot,
      selectedChecks: ["deps", "package"],
      profile: "node-cli-package",
    });
    expect(scoped.isFullReleaseGate).toBe(false);
  }, 30_000);
});
